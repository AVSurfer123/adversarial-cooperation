...
class BasicMAC:
    ...
    def jsma(self, inputs, target_class, agent_idx = 0, max_distortion=0.1):

        # Make a clone since we will alter the values
        input_features = th.autograd.Variable(inputs.clone(), requires_grad=True)
        ...
        source_class = th.argmax(output[0])
        theta = 0.1
        max_theta = 0.9 # time running should not depend on this
        k = 0
        max_iter = 20
        while source_class != target_class and k < max_iter:
            jacobian = self.compute_jacobian(input_features, output)
            i,j,d = self.select_best_saliency_map(jacobian, num_features, target_class)
    ...
    def compute_jacobian(self, inputs, output):
        ...
        jacobian = th.zeros(num_classes, *inputs.size())
        grad_output = th.zeros(*output.size())
        if inputs.is_cuda:
    ...
    def select_best_saliency_map(self, jacobian, num_features, target_index):
        jacobian = jacobian.squeeze(0)
        all_sum = th.sum(jacobian, 0) # sum all q partial derivatives for each feature
